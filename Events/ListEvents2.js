$(document).ready(function listEvents() {
        ...
        $events2.html("<i>Loading events, please stand by...</i>");
        $.get('https://www.eventbriteapi.com/v3/events/search/?token='+token+'&location.address=melbourne,australia&sort_by=date&categories=103&expand=venue', 
            function(res) {
            if(res.events2.length) {
                var s = "";
                for(var i=0;i<res.events2.length;i++) {
                    var event = res.events2[i];
                ...
                $events2.html("<p>Sorry, there are no upcoming events.</p>");
            }
        });
